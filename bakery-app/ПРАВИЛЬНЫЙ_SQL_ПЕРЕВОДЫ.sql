-- ============================================
-- ПРАВИЛЬНЫЙ SQL ДЛЯ ЗАПОЛНЕНИЯ ПЕРЕВОДОВ
-- ============================================
-- Этот скрипт работает с ЛЮБЫМИ ID, ищет товары по названию

-- ШАГ 1: Проверьте структуру вашей таблицы
SELECT column_name, data_type 
FROM information_schema.columns 
WHERE table_name = 'products';

-- Если колонка называется 'title' вместо 'name', замените везде 'name' на 'title'
-- Если колонок name_en нет, сначала создайте их (см. конец файла)

-- ============================================
-- ЗАПОЛНЕНИЕ ПЕРЕВОДОВ ПО НАЗВАНИЮ
-- ============================================
...
WHERE name LIKE '%атск%' OR name LIKE '%Датск%';
...
WHERE name LIKE '%расн%бархат%' OR name LIKE '%Красн%бархат%';
...
WHERE name LIKE '%околадн%торт%' OR name LIKE '%Шоколадн%торт%';
...
WHERE name LIKE '%иабатт%' OR name LIKE '%Чиабатт%';
...
WHERE name LIKE '%леб%ржан%' OR name LIKE '%Хлеб%ржан%';
...
WHERE name LIKE '%анна%котт%' OR name LIKE '%Панна%котт%';
...
-- ============================================
-- ЕСЛИ КОЛОНОК НЕТ, СОЗДАЙТЕ ИХ СНАЧАЛА
-- ============================================
/*
